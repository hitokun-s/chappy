...
def download_mnist_data():
    ...
    request.urlretrieve('{:s}/{:s}'.format(parent, train_images), 'data/' + train_images)
    print('Done')
    print('Downloading {:s}...'.format(train_labels))
    request.urlretrieve('{:s}/{:s}'.format(parent, train_labels), 'data/' + train_labels)
    print('Done')
    print('Downloading {:s}...'.format(test_images))
    request.urlretrieve('{:s}/{:s}'.format(parent, test_images), 'data/' + test_images)
    print('Done')
    print('Downloading {:s}...'.format(test_labels))
    request.urlretrieve('{:s}/{:s}'.format(parent, test_labels), 'data/' + test_labels)
    print('Done')

    print('Converting training data...')
    data_train, target_train = load_mnist('data/' + train_images, 'data/' + train_labels,
                                          num_train)
    print('Done')
    print('Converting test data...')
    data_test, target_test = load_mnist('data/' + test_images, 'data/' + test_labels, num_test)
    mnist = {}
    ...
    print('Done')
    print('Save output...')
    with open('data/mnist.pkl', 'wb') as output:
